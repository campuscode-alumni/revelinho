...
feature 'Positions list' do
  ...
  scenario 'Employee hides position details' do
    company = create(:company, url_domain: 'revelo.com.br')
    employee = create(:employee, company: company)
    create(:position, title: 'Engenheiro de Software Pleno',
                      company: company,
    ...
    expect(page).not_to have_content('Artista de commits')
    expect(page).not_to have_content('3500')
    expect(page).not_to have_content('5000')
    expect(page).not_to have_content('Tecnologia e serviços')
  end
